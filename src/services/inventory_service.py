...
class InventoryService:
    ...
    # def insert_inventory_with_checks(
    #                                     self,
    #                                     db: Session,
    #                                     product_id: int,
    #                                     ware_house_id: int,
    ...
    #             # self.check_product(db, product_id)

    #             product = ProductRepository().get_product_by_id(db, product_id)
    #             if not product:
    #                 raise HTTPException(detail=f"Product with ID {product_id} does not exist.")


    #             warehouse = WarehouseRepository().get_warehouse_by_id(db, ware_house_id)
    #             if not warehouse:
    #                 raise HTTPException(detail=f"Warehouse with ID {ware_house_id} does not exist.")

    #             productcount_repo = ProductCountRepository()
    #             productcount = productcount_repo.get_product_amount(db, product_id, ware_house_id=ware_house_id)

    #             if productcount < quantity and not kind:
    #                 raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
    #                                     detail=f"Product amount shuold be more than quantiy.")
    #             else:
    #                 if kind:
    #                     productcount_repo.update_product_amount(db, product_id, ware_house_id, quantity)
    #                 else:
    #                     productcount_repo.update_product_amount(db, product_id, ware_house_id, -quantity)
    #             # Check if the receiver person exists
    #             receiver_person = PersonRepository().get_person_by_id(db, receiver_id)
    #             if not receiver_person:
    #                 raise HTTPException(detail=f"Receiver person with ID {receiver_id} does not exist.")

    #             # Check if the delivery person exists
    #             delivery_person = PersonRepository().get_person_by_id(db, deliver_id)
    #             if not delivery_person:
    #                 raise HTTPException(detail=f"Delivery person with ID {deliver_id} does not exist.")

    #             if date_time is not None:
    #                 create_time = string_to_time_foramt(date_time)
    #                 result = self.repository.insert_inventory(
    #                                                             db,
    #                                                             product_id,
    #                                                             ware_house_id,
    #                                                             receiver_id,
    ...
    #                 # result.created_at = time_format_to_string(result.created_at)
    #                 return result
    #             else:
